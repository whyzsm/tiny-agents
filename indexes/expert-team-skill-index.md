...
| T-MAX 后端架构专家团 / T-MAX backend architecture expert team | [`tech-tmax-backend-architect-team`](../skills/tech-tmax-backend-architect-team/SKILL.md) | `skills/tech-tmax-backend-architect-team/` | 采用单 Agent + 规则包：`tmax-backend-dispatch-agent` 按真实仓库证据顺序执行六条规则轨道，控制权限门禁和有界小循环；不创建子 Agent，不把静态检查当运行时证明 / Uses one dispatch Agent plus one rule package to execute six rule tracks sequentially with authority gates and bounded loops; it creates no child Agents and never treats static checks as runtime proof | 从 `tech-backend-architect-team` 独立复制升等；规则包内置脱敏 T-MAX 栈画像、调度协议和验证矩阵，可选 Skill 仅在同一 Agent 上下文加载 / independently adapted copy with a sanitized stack profile, dispatch protocol, and validation matrix; optional Skills are loaded only in the same Agent context |
...
`harmony-expert-team`、`design-engine`、`design-product-strategy-team`、`motion-expert-team` 和 `tech-tmax-backend-architect-team` 有意同时登记在两个索引中：`expert-team-file-list.md` 登记固定专家团入口，本文件登记自建或适配 router Skill 及其实现边界。
...
## tech-tmax-backend-architect-team

- **Skill 类型 / Skill type**：供单个 T-MAX 后端调度 Agent 使用的仓库规则包 / repository rule package for one T-MAX backend dispatch Agent
- **来源边界 / Source boundary**：从 `tech-backend-architect-team` 独立复制升等；原包保持不变，本包不依赖 WorkBuddy 隐藏 Prompt、私有地址、凭据或本机绝对路径 / independently adapted from a copy of `tech-backend-architect-team`; the original remains unchanged and this package does not depend on hidden WorkBuddy prompts, private addresses, credentials, or local absolute paths
- **实现模式 / Implementation mode**：`single-agent-with-rule-package`；`tmax-backend-dispatch-agent` 并发度为 1、禁止委派，六条 track 是同一 Agent 顺序执行的规则路径，不是成员、子 Agent 或独立顶层 Skill / one non-delegating Agent executes all six rule tracks sequentially; tracks are not members, child Agents, or standalone Skills
- **小循环 / Small loop**：每条 track 使用 `observe -> hypothesize -> plan-one-step -> act -> verify -> decide`，默认最多三轮且每轮必须产生证据增量 / each track uses a bounded evidence-producing loop with at most three iterations by default
- **技术栈范围 / Stack scope**：覆盖 Java 8、Maven 多模块、Spring Boot/Kylin、MyBatis XML、TiDB/MySQL、Apollo/Nacos、Feign、AnePageHelper、UEP/EasyExcel、Redis/RabbitMQ 等常见 T-MAX 后端表面；每次仍必须核对目标仓库 / covers common T-MAX backend surfaces while requiring target-repository verification on every task
- **证据契约 / Evidence contract**：区分 `source_observed`、`static_validated`、`build_validated`、`test_validated`、`runtime_verified`、`runtime_blocked` 和 `delivery_verified`，不得把声明、配置或静态通过当业务执行证明 / keeps source, static, build, test, runtime, blocked, and delivery evidence distinct
- **可选伴随能力 / Optional companions**：环境存在时可在同一 Agent 上下文加载 `tmax-backend-stack`、`system-architect`、`api-dev`、代码评审、排障和安全 Skill；不得把它们调度为成员 / may load narrower Skills in-process but never dispatches them as members
- **权限边界 / Authority boundary**：本地源修改、静态检查、编译、测试、实时数据库/API/MQ/配置调用、提交、推送、部署和发布保持独立授权 / keeps source edits, static checks, compile, tests, live calls, commit, push, deployment, and release as separate authority classes
