...
```text
$ecommerce-user-profiling 帮我完成一次用户画像，先判断需要哪些子技能
$ecommerce-user-profiling 根据这些资料生成完整交付物，并说明风险和假设
$ecommerce-user-profiling 只处理“行为数据接入与口径核对（输入层）”，给出结果和下一步建议
$ecommerce-user-profiling 把“通用客户分层与精准营销分群（分析层 - 主分群）”的结果整理成可交付报告
```
