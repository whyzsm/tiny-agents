...
description: >-
  中国个人信息保护法（PIPL）合规检查、风险评估和文档生成工具。 为企业提供全面的PIPL合规解决方案。 Use when: 需要进行PIPL合规自查、个人信息处理风险评估、
  合规文档生成、企业合规管理、数据处理影响评估、跨境传输合规检查。 🎉 v1.1.9 重要更新： - 🔧 全新统一化CLI接口，与GDPR、CCPA工具体验一致 - 📊
  支持JSON/Markdown/HTML/CSV多格式报告输出 - 🚀 跨法域检查：可与GDPR、CCPA配合使用 - 🧹 精简核心文件，移除冗余演示脚本
  触发关键词：PIPL、个人信息保护法、合规检查、风险评估、 隐私合规、数据保护、跨境传输、影响评估 适用范围：中华人民共和国个人信息保护法（PIPL）
  运行模式：纯本地，无网络请求 ❎ 外部依赖：Python标准库 + pandas（可选）
---
