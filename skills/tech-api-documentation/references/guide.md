...
`$tech-api-documentation` API 文档工作流入口

图片中的 `tech-api-documentation-codex` 在本仓库映射为真实入口 `$tech-api-documentation`。

## 触发的技能

| 技能 | 能力 |
|---|---|
| `$tech-api-documentation` | 完整 API 文档工作流总入口 |
...
1. API 契约设计
   ...
   调用 `tech-api-documentation` 的契约设计阶段
...
```text
$tech-api-documentation 为这组 REST 路由生成 OpenAPI 3.0 和 Markdown API 文档

$tech-api-documentation 设计一个用户、订单和支付 API，并包含分页、认证和错误格式

$tech-api-documentation 检查这些接口实现和现有文档是否一致，列出差异

$tech-api-documentation 为这个 API 输出 SDK 快速开始、curl 示例和 Postman 验证集合
```
