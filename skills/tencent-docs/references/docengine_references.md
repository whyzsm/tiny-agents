...
| 调用方式 | 通过 MCP 协议调用 doc-mcp 服务，使用与 tencent-docs 共享的 Authorization Token |
| Token    | 与 tencent-docs / slide-mcp / sheet-mcp 共用同一 Token，完成授权（`references/auth.md`）后自动配置 |
| 文档类型 | 仅支持 Word 文档类型（`doc_type: word`）                                      |

> ⚠️ **所有 `doc.*` 工具均通过 `doc-mcp` 独立 MCP 服务调用**（`mcporter call "doc-mcp" "doc.<工具名>"`），与 `tencent-docs` 主服务平级。Token 共用，但工具调用走独立 endpoint。
